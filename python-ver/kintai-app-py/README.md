...
4. [モジュール別仕様](#4-モジュール別仕様)（4.12 `calendar_widget` 追加）
5. [機能仕様](#5-機能仕様)
6. [データフロー](#6-データフロー)
...
### 4.12 `assets/calendar_widget.py` — カスタムカレンダー

#### `CalendarCell` クラス

`QLabel` サブクラス。1つの日付セルを表す。選択・今日・祝日・土曜・ホバーの各状態に応じてスタイルシートを動的に切り替える。テーマ色はクラス変数で保持し `update_theme_colors()` で一括更新する。

#### `CalendarWidget` クラス

日曜始まりの日本語カスタムカレンダー Widget。起動時から今日を自動選択し、1分ごとのタイマーで日付変更を検出して自動切り替えする。

| メソッド | 説明 |
|---|---|
| `select_date(d)` | 指定日付を選択して `date_selected` シグナルを発火する。月が異なる場合は表示月も自動移動 |
| `get_selected_date()` | 現在選択中の日付を返す |
| `set_month(year, month)` | 表示月を変更してグリッドを再構築する |
| `prev_month()` / `next_month()` | 前後月に移動する |
| `set_theme(theme)` | テーマを切り替えてグリッドを再描画する |
| `_check_date_rollover()` | タイマーから毎分呼ばれる。システム日付が `_last_known_today` と異なれば `select_date(新しい今日)` を呼ぶ |

#### 日付自動切り替えの仕組み

```
起動時: _selected_date = today, _last_known_today = today
   ↓ (1分ごと)
_check_date_rollover() が get_today() を呼び _last_known_today と比較
   ↓
変化なし → return（何もしない・負荷ゼロ）
変化あり → _last_known_today を更新 → select_date(新しい今日)
```

- 日付変更から最大1分以内に切り替わる
- ユーザーが別の日付を選択中でも、日付変更時は今日に戻る

#### シグナル

| シグナル | 型 | タイミング |
|---|---|---|
| `date_selected` | `date` | セルクリック・`select_date()` 呼出時 |
| `date_double_clicked` | `date` | セルダブルクリック時（一括記入への追加に使用） |
